...
function Remove-RabbitMQQueue
{
    ...
    Process
    {
        if ($pscmdlet.ShouldProcess("server: $ComputerName, vhost: $VirtualHost", "Remove queue(s): $(NamesToString $Name '(all)')"))
        {
            foreach($n in $Name)
            {
                $url = "$(Format-BaseUrl -ComputerName $ComputerName -port $port -useHttps:$useHttps)api/queues/$([System.Web.HttpUtility]::UrlEncode($VirtualHost))/$([System.Web.HttpUtility]::UrlEncode($n))"
