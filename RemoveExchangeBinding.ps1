...
function Remove-RabbitMQExchangeBinding
{
    ...
    Process
    {
        if ($pscmdlet.ShouldProcess("$ComputerName/$VirtualHost", "Remove binding between source exchange $ExchangeName and destination exchange $Name"))
        {
            $url = "$(Format-BaseUrl -ComputerName $ComputerName -port $port -useHttps:$useHttps)api/bindings/$([System.Web.HttpUtility]::UrlEncode($VirtualHost))/e/$([System.Web.HttpUtility]::UrlEncode($ExchangeName))/e/$([System.Web.HttpUtility]::UrlEncode($Name))/$([System.Web.HttpUtility]::UrlEncode($RoutingKey))"
            ...
            Write-Verbose "Removed binding between source exchange $ExchangeName and destination exchange $Name $n on $ComputerName/$VirtualHost"
            $cnt++
        }
    }
