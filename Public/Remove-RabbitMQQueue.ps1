...
function Remove-RabbitMQQueue
{
    ...
    Process
    {
        if ($pscmdlet.ShouldProcess("server: $HostName, vhost: $VirtualHost", "Remove queue(s): $(NamesToString $Name '(all)')"))
        {
            foreach($n in $Name)
            {
                $url = "$(Format-BaseUrl -HostName $HostName -port $port -useHttps:$useHttps)api/queues/$([System.Web.HttpUtility]::UrlEncode($VirtualHost))/$([System.Web.HttpUtility]::UrlEncode($n))"
