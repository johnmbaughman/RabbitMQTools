...
function Add-RabbitMQVirtualHost
{
    ...
    Process
    {
        if (-not $pscmdlet.ShouldProcess("server: $HostName", "Add vhost(s): $(NamesToString $Name '(all)')")) {
            foreach ($qn in $Name) 
            { 
        ...
        foreach($n in $Name)
        {
            ...
            Invoke-RestMethod $url -Credential $Credentials -AllowEscapedDotsAndSlashes -DisableKeepAlive:$InvokeRestMethodKeepAlive -ErrorAction Continue -Method Put -ContentType "application/json" -SkipCertificateCheck:$skipCertificateCheck
