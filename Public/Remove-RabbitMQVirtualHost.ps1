...
function Remove-RabbitMQVirtualHost
{
    ...
    Process
    {
        if (-not $pscmdlet.ShouldProcess("server: $HostName", "Remove vhost(s): $(NamesToString $Name '(all)')")) {
            foreach ($qn in $Name)
            { 
        ...
        foreach($n in $Name)
        {
            $url = "$(Format-BaseUrl -HostName $HostName -port $port -useHttps:$useHttps)api/vhosts/$([System.Web.HttpUtility]::UrlEncode($n))"
            Invoke-RestMethod $url -Credential $Credentials -AllowEscapedDotsAndSlashes -DisableKeepAlive:$InvokeRestMethodKeepAlive -ErrorAction Continue -Method Delete -ContentType "application/json" -SkipCertificateCheck:$skipCertificateCheck
